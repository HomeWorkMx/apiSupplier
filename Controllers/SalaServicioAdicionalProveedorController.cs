...
namespace apiSupplier.Controllers
{
    ...
    public class SalaServicioAdicionalProveedorController : Controller
    {
        ...
        [HttpGet("SalaServicioAdicionalProveedorGetByIdProveedor")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IEnumerable<SalaServicioAdicionalProveedorDto>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(ProblemDetails))]
        [ProducesResponseType(StatusCodes.Status404NotFound, Type = typeof(NotFoundResult))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ProblemDetails))]
        public async Task<ActionResult<IEnumerable<SalaServicioAdicionalProveedorDto>>> SalaServicioAdicionalProveedorGetByIdProveedor(int idProveedor)
        {
            var entidades = await _clientMsSala.SalaServicioAdicionalProveedorGetByIdProveedorAsync(idProveedor);
            /*var entidades = await
                _memoryCache.GetOrCreateAsync("SalaServicioAdicionalProveedorGetByIdProveedorAsync" + idProveedor.ToString(), entry =>
                {
                    entry.AbsoluteExpiration = DateTime.Now.AddMinutes(5);
                    entry.Priority = CacheItemPriority.Normal;
                    return _clientMsSala.SalaServicioAdicionalProveedorGetByIdProveedorAsync(idProveedor);
                });*/
